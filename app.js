...
const dateToWeekIdx = str => {
  ...
  for(let i=0;i<WEEKS.length;i++){
    ...
    const sy=s.getFullYear(),sm=s.getMonth(),sd=s.getDate(),ey=e.getFullYear(),em=e.getMonth(),ed=e.getDate();
    if(((y>sy)||(y===sy&&mo>sm)||(y===sy&&mo===sm&&dy>=sd))&&((y<ey)||(y===ey&&mo<em)||(y===ey&&mo===em&&dy<=ed))) return i;
  }
  return -1;
};
...
const parseFcInflows = text => {
  ...
  const hdrWi=wkHdrs.map(h=>{const d=parseDate(h);if(!d)return -1;for(let i=0;i<WEEKS.length;i++){const s=new Date(WEEKS[i].s),e=new Date(WEEKS[i].e);const y=d.getFullYear(),mo=d.getMonth(),dy=d.getDate(),sy=s.getFullYear(),sm=s.getMonth(),sd=s.getDate(),ey=e.getFullYear(),em=e.getMonth(),ed=e.getDate();if(((y>sy)||(y===sy&&mo>sm)||(y===sy&&mo===sm&&dy>=sd))&&((y<ey)||(y===ey&&mo<em)||(y===ey&&mo===em&&dy<=ed)))return i;}return -1;});
  ...
  lines.slice(1).forEach(line=>{
    ...
    if(sec==="tox"&&(lbl==="new"||lbl.includes("re-occur"))){wkHdrs.forEach((_,hi)=>{const wi=hdrWi[hi];if(wi<0||!WEEKS[wi].fc)return;const q=parseAmt(cols[hi+2]);if(q)toxAcc[wi]=(toxAcc[wi]||0)+q;});return;}
    if(sec==="wl"&&(lbl==="new"||lbl.includes("re-occur"))){wkHdrs.forEach((_,hi)=>{const wi=hdrWi[hi];if(wi<0||!WEEKS[wi].fc)return;const q=parseAmt(cols[hi+2]);if(q)wlAcc[wi]=(wlAcc[wi]||0)+q;});return;}
  });
...
const fetchSheet = async gid => {
  const res=await fetch(`${MAKE_WEBHOOK}?gid=${gid}`,{method:"GET"});
  if(!res.ok) throw new Error(`HTTP ${res.status}`);
  const text=await res.text(); if(!text||text.length<5) throw new Error("Empty response");
  return text;
};
...
function App() {
  ...
  const confirmUnk=()=>{
    ...
    setSyncMsg(`✓ ${pending.type==="anb"?"ANB":"Invoicing"} synced`);
  ...
  const modalWrap=content=>el("div",{style:{position:"fixed",inset:0,background:"rgba(0,0,0,0.28)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}},
  ...
  return el("div",{style:{fontFamily:"system-ui,sans-serif",background:S.bg,minHeight:"100vh",color:"#1e293b"}},
    el("div",{style:{padding:"10px 16px",borderBottom:"1px solid #e2e8f0",background:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}},
      el("div",null,el("div",{style:{fontSize:10,color:S.muted,textTransform:"uppercase",letterSpacing:1}},"Restorative Injectables — Denver"),el("div",{style:{fontSize:17,fontWeight:700}},"Cash Flow Worksheet")),
    ...
    pasteModal&&modalWrap(el("div",null,
      ...
      el("div",{style:{marginBottom:12}},el("div",{style:{fontSize:12,fontWeight:700,color:S.red,marginBottom:4}},"📉 Expenses tab"),el("textarea",{value:pasteExp,onChange:e=>setPasteExp(e.target.value),rows:5,placeholder:"Paste forecast expenses here...",style:{width:"100%",padding:8,borderRadius:8,border:"1px solid #fecaca",fontSize:11,boxSizing:"border-box",fontFamily:"monospace",resize:"vertical",outline:"none"}})),
      el("div",{style:{marginBottom:16}},el("div",{style:{fontSize:12,fontWeight:700,color:S.purple,marginBottom:4}},"📈 Assumptions tab"),el("textarea",{value:pasteInf,onChange:e=>setPasteInf(e.target.value),rows:5,placeholder:"Paste assumptions here...",style:{width:"100%",padding:8,borderRadius:8,border:"1px solid #ddd6fe",fontSize:11,boxSizing:"border-box",fontFamily:"monospace",resize:"vertical",outline:"none"}})),
    ...
    unkModal&&modalWrap(el("div",null,
      ...
      el("div",{style:{overflowX:"auto"}},el("table",{style:{borderCollapse:"collapse",width:"100%",fontSize:12}},
        ...
        el("tbody",null,...pending.unk.map((row,idx)=>{
          const amt=parseAmt(row["Amount"]||row["Total Due"]),oi=pending.rows.indexOf(row);
          return el("tr",{key:idx,style:{borderBottom:"1px solid #f1f5f9"}},
            ...
            el("td",{style:{padding:"5px 10px"}},el("select",{value:manMap[oi]||"",onChange:e=>setManMap(m=>({...m,[oi]:e.target.value})),style:{width:"100%",padding:"4px 8px",borderRadius:6,border:"1px solid #e2e8f0",fontSize:12,background:"#fff"}},el("option",{value:""},"— Skip —"),...ALL_LINES.map(l=>el("option",{key:l,value:l},l))))
          );
        }))
      )),
    ...
    tab==="variance"&&el("div",{style:{padding:16}},
      !fcSnapshot
        ...
        :el("div",null,
            ...
            el("div",{style:{overflowX:"auto"}},el("table",{style:{borderCollapse:"collapse",minWidth:"100%",fontSize:12}},
              ...
              el("tbody",null,...varRows.map((row,ri)=>el("tr",{key:ri,style:{borderBottom:"1px solid #f1f5f9",background:row.sub?"#fafafa":"#fff"}},
                ...
                ...varWeeks.flatMap(w=>{
                  ...
                  return [el("td",{key:`f${w.i}`,style:{padding:"3px 8px",textAlign:"right",background:S.pBg,borderLeft:"1px solid #e2e8f0",color:S.purple}},fmt(fc)),el("td",{key:`a${w.i}`,style:{padding:"3px 8px",textAlign:"right",background:S.bBg,borderLeft:"1px solid #e2e8f0",color:"#1d4ed8",fontWeight:500}},fmt(ac)),el("td",{key:`d${w.i}`,style:{padding:"3px 8px",textAlign:"right",background:"#f8fafc",borderLeft:"1px solid #e2e8f0",color:dc,fontWeight:600,fontSize:11}},fmtD(delta))];
                })
              )))
            ))
          )
